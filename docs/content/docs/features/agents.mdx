...
- `response.shell_call.in_progress` — spec.
- `response.shell_call.command_started` — **Hadrian extension.** Resolved command string.
- `response.shell_call.output_chunk` — **Hadrian extension.** stdout / stderr deltas.
- `response.shell_call.file_created` — **Hadrian extension.** One event per captured file
  under `/mnt/data`.
- `response.shell_call.completed` — spec, with `exit_code`, plus the Hadrian-extension
  outcome fields `duration_ms`, `killed`, and `max_output_truncated`.
- `response.output_item.done` carrying a `shell_call_output` item — spec. The item adds the
  same outcome fields plus the Hadrian-extension `output_files` array.

SDKs that hook OpenAI's typed event stream resolve `in_progress` / `completed` /
`output_item.done` to their built-in event types; the Hadrian-extension events fall through
as untyped JSON the consumer can opt into.
